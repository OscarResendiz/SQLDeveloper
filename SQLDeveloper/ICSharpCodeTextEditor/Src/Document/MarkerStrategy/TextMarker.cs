...
namespace ICSharpCode.TextEditor.Document
{
    ...
    public class TextMarker : ISegment
    {
        ...
        {
            return String.Format("[TextMarker: Offset = {0}, Length = {1}, Type = {2}, Color={3}, Linea={4}]",
                                 offset,
                                 length,
                                 textMarkerType,
                                 color,
		...
		public TextMarker(int offset, int length, TextMarkerType textMarkerType, Color color, Color foreColor,int ln=-1)
		{
			if (length < 1) length = 1;
			this.offset          = offset;
			this.length          = length;
