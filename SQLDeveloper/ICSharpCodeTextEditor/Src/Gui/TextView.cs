...
namespace ICSharpCode.TextEditor
{
    ...
    public class TextView : AbstractMargin, IDisposable
    {
        ...
        Brush GetBgColorBrush(int lineNumber)
        {
            ...
            #region Oscar
            IList<TextMarker> markers1 = Document.MarkerStrategy.GetMarkers(0, 0, lineNumber);
            foreach (TextMarker mobj in markers1)
            {
                if (mobj.TextMarkerType == TextMarkerType.Linea && mobj.LineNumber == lineNumber)
                {
                    return BrushRegistry.GetBrush(mobj.Color);
                }
            }
            #endregion
        ...
        Brush GetMarkerLineBrush(Graphics g, TextMarker marker)
        {
            if (marker.TextMarkerType == TextMarkerType.Linea)
            {
                Brush br = BrushRegistry.GetBrush(marker.Color);
                return br;
            }
            return null;
        }
        ...
        Brush GetMarkerBrush(IList<TextMarker> markers, ref Color foreColor)
        {
            ...
            {
                if (marker.TextMarkerType == TextMarkerType.SolidBlock || marker.TextMarkerType == TextMarkerType.Linea)
                {
        ...
        int PaintLinePart(Graphics g, int lineNumber, int startColumn, int endColumn, Rectangle lineRectangle, int physicalXPos)
        {
            ...
            Brush BrushMarkerLine = null;
            bool MarkerLinea = false;
            HighlightColor selectionColor = textArea.Document.HighlightingStrategy.GetColorFor("Selection");
            ColumnRange selectionRange = textArea.SelectionManager.GetSelectionAtLine(lineNumber);
            ...
            #region Recorrido de palabra por palabra
            for (int wordIdx = 0; wordIdx < currentLine.Words.Count; wordIdx++)
            {
                ...
                {
                    // TODO: Tal vez tengamos que dividir en startColumn cuando apoyamos marcadores de pliegue
                    // inside words
            ...
            repeatDrawCurrentWord:
                ...
                IList<TextMarker> markers = Document.MarkerStrategy.GetMarkers(currentLine.Offset + currentWordOffset, currentWord.Length, lineNumber);
                Brush wordBackBrush = GetMarkerBrush(markers, ref wordForeColor); //se trae el color de las letrass

                #region Es posible que tengamos que dividir la palabra actual porque un marcador / la selección comienza / termina dentro de ella
