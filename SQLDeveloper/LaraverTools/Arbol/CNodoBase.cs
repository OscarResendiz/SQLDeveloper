...
namespace LaraverTools.Arbol
{
    public class CNodoBase : TreeNode
    {
        ...
        protected void DeleteMenuItem(string menu)
        {
            foreach (System.Windows.Forms.ToolStripItem MenuItem in MenuPrinciapl.Items)
            {
                if (MenuItem.GetType() == typeof(System.Windows.Forms.ToolStripMenuItem))
                {
                    if (MenuItem.Text == menu)
                    {
                        MenuPrinciapl.Items.Remove(MenuItem);
                        return;
                    }
                }
            }
...
*/
        ...
        protected Formularios.FormLaravel GetFormFormLaravel()
        {
            System.Windows.Forms.Control obj = this.TreeView;
            while (obj.GetType() != typeof(Formularios.FormLaravel))
            {
                obj = obj.Parent;
            }
            Formularios.FormLaravel form = (Formularios.FormLaravel)obj;
            return form;
        }
    }
}
