...
namespace SQLDeveloper.Modulos.Visores.Tabla
{
    public class CNodoCampo: CNodoBase
    {
        ...
        public CNodoCampo(CTabla tabla, CCampo campo, MotorDB.IMotorDB db)
            : base(db)
        {
            Tabla = tabla;
            Campo = campo;
        ...
        public override CVisorBase GetVisor()
        {
            if (Visor == null)
            {
                Visor = new CVisorCampo(Tabla, Campo,motor);
            }
            return Visor;
        }
        ...
        {
            FormEditarColumna dlg = new FormEditarColumna(Tabla.Nombre, motor);
