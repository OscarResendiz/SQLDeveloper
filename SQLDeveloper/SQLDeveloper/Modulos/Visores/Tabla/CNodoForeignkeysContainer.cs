...
namespace SQLDeveloper.Modulos.Visores.Tabla
{
    public class CNodoForeignkeysContainer: CNodoFolder
    {
        ...
        public CNodoForeignkeysContainer(CTabla tabla, MotorDB.IMotorDB db)
            : base(db)
        {
           // base.CNodoFolder();
            Tabla = tabla;
        ...
        {
            Tabla = motor.DameTabla(Tabla.Nombre);
            Nodes.Clear();
            foreach(CForeignKey obj in Tabla.ForeignKeys)
            {
                CNodoFk fk = new CNodoFk(Tabla, obj,motor);
                Nodes.Add(fk);
            }
        }
        public override CVisorBase GetVisor()
        {
            if (Visor == null)
            {
                Visor = new CVisorFKS(Tabla.Nombre,motor);
        ...
        {
            FormAddForeignKey dlg = new FormAddForeignKey(Tabla.Nombre, motor);
