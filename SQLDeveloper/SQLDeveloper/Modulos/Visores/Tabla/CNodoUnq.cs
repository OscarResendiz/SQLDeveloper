...
namespace SQLDeveloper.Modulos.Visores.Tabla
{
    public class CNodoUnq: CNodoBase
    {
        ...
        public CNodoUnq(CTabla tabla, CUnique unique, MotorDB.IMotorDB db)
            : base(db)
        {
            Tabla = tabla;
            Unique = unique;
        ...
        private void MenuEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("¿Seguro que desea eliminar la clave unica: " + Unique.Nombre + "?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    return;
                motor.AlterTable_DropUnique(Tabla.Nombre, Unique.Nombre);
            }
        ...
        {
            if (Visor == null)
                Visor = new CVisorUnico(Unique,motor);
            return Visor;
        }
    }
}
