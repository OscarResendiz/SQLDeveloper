...
namespace SQLDeveloper.Modulos.Visores.Tabla
{
    public partial class FormAddForeignKey : Form
    {
        ...
        public FormAddForeignKey(string tablaHija,MotorDB.IMotorDB db)
        {
            motor = db;
            InitializeComponent();
            TTablaHija.Text = tablaHija;
        }
        private void LLenaComboTablas()
        {
            List<MotorDB.CObjeto> l = motor.Buscar("", MotorDB.EnumTipoObjeto.TABLE);
        ...
        private void LLenaAccionesReferenciales()
        {
            ...
            la = motor.DameAccionesReferencialesActualizadoValidos();
            ld = motor.DameAccionesReferencialesBorradoValidos();
            //limpio los combos
...
//            CargaCamposTablaHija();
        ...
        private void BTablas_Click(object sender, EventArgs e)
        {
            ...
            FormBuscadorTablas dlg = new FormBuscadorTablas(motor);
        ...
        {
            //me traigo la tabla
            MotorDB.CTabla Tabla = motor.DameTabla(padre);
            if(Tabla==null)
            {
                return;
            }
        ...
        private void AgregaTipo(MotorDB.CTipoDato tipo, int tamaño)
        {
            ...
            if(tipo.UsaLongitud!= MotorDB.TIPO_LONGITUD.NOREQUERIDO)
            {
                if (tipo.UsaLongitud == MotorDB.TIPO_LONGITUD.OBLIGATORIO)
                {
                    s = s + "(" + tamaño + ")";
                }
                else if (tipo.UsaLongitud != MotorDB.TIPO_LONGITUD.OPCIONAL)
                {
                    if (tamaño > 1)
                    {
                        s = s + "(" + tamaño + ")";
        ...
        private void CargaCamposTablaHija()
        {
            ...
            MotorDB.CTabla tabla = motor.DameTabla(TTablaHija.Text);
        ...
        private void  AsignaNombreFK()
        {
            ...
            do
            {
                ...
                List<MotorDB.CObjeto> l = motor.Buscar(nombre, MotorDB.EnumTipoObjeto.FOREIGNKEY);
        ...
        private void BAceptar_Click(object sender, EventArgs e)
        {
            ...
            MotorDB.CTabla tablaPadre = motor.DameTabla(ComboTablas.Text);
            //asigno los campos
            foreach(DataRow dr in dt.Rows)
            {
